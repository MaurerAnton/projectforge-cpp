...
// ============================================================================
// PollPagesRest — Complete REST API Implementation
...
static bool validateRequest(const drogon::HttpRequestPtr& req, nlohmann::json& outBody) {
    ...
    try {
        ...
        spdlog::error("[PollPagesRest] Invalid JSON: {}", e.what());
        return false;
    }
}
...
static nlohmann::json handleGetList(const drogon::HttpRequestPtr& req) {
    ...
    spdlog::info("[PollPagesRest] GET list: page={}, pageSize={}", filter["page"], filter["pageSize"]);
    return result;
}
...
static nlohmann::json handleGetById(int64_t id) {
    ...
    spdlog::info("[PollPagesRest] GET by id: {}", id);
    return result;
}
...
static nlohmann::json handleCreate(const nlohmann::json& body) {
    ...
    spdlog::info("[PollPagesRest] Created new entity");
    return result;
}
...
static nlohmann::json handleUpdate(int64_t id, const nlohmann::json& body) {
    ...
    spdlog::info("[PollPagesRest] Updated entity id={}", id);
    return result;
}
...
static nlohmann::json handleDelete(int64_t id) {
    ...
    spdlog::info("[PollPagesRest] Deleted entity id={}", id);
    return result;
}
...
static struct PollPagesRestAutoReg {
    PollPagesRestAutoReg() {
        spdlog::info("[PollPagesRest] Registering routes at /api/v1//polls");
        PollPagesRest::registerRoutes();
    }
} _PollPagesRestAutoReg;
