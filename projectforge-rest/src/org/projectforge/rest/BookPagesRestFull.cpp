...
// ============================================================================
// BookPagesRest — Complete REST API Implementation
...
static bool validateRequest(const drogon::HttpRequestPtr& req, nlohmann::json& outBody) {
    ...
    try {
        ...
        spdlog::error("[BookPagesRest] Invalid JSON: {}", e.what());
        return false;
    }
}
...
static nlohmann::json handleGetList(const drogon::HttpRequestPtr& req) {
    ...
    spdlog::info("[BookPagesRest] GET list: page={}, pageSize={}", filter["page"], filter["pageSize"]);
    return result;
}
...
static nlohmann::json handleGetById(int64_t id) {
    ...
    spdlog::info("[BookPagesRest] GET by id: {}", id);
    return result;
}
...
static nlohmann::json handleCreate(const nlohmann::json& body) {
    ...
    spdlog::info("[BookPagesRest] Created new entity");
    return result;
}
...
static nlohmann::json handleUpdate(int64_t id, const nlohmann::json& body) {
    ...
    spdlog::info("[BookPagesRest] Updated entity id={}", id);
    return result;
}
...
static nlohmann::json handleDelete(int64_t id) {
    ...
    spdlog::info("[BookPagesRest] Deleted entity id={}", id);
    return result;
}
...
static struct BookPagesRestAutoReg {
    BookPagesRestAutoReg() {
        spdlog::info("[BookPagesRest] Registering routes at /api/v1//books");
        BookPagesRest::registerRoutes();
    }
} _BookPagesRestAutoReg;
