...
public:
    ...
    std::vector<CommonEntry> getByStatus(const std::string& s) {
        auto all=getAll(); std::vector<CommonEntry> r;
        for(auto& e:all) if(!e.deleted && e.status==s) r.push_back(e);
        return r;
    }
    
    std::vector<CommonEntry> getRecent(int limit=50) {
        auto all=getAll();
...
class CommonService {
public:
    static CommonService& instance() { static CommonService svc; return svc; }
    
    void init() { initialized_=true; }
    bool isInitialized() const { return initialized_; }
    
    bool processEntry(const CommonEntry& entry) {
        spdlog::info("common: Processing entry: {}", entry.title);
        return true;
    }
    
    std::string getStatus() const {
        return "{ \"domain\": \"" + std::string("common") + "\", \"status\": \"OK\" }";
    }
