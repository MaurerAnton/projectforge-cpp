...
#pragma once
// DVelop integration for document management
...
public:
    ...
    std::vector<DvelopEntry> getByStatus(const std::string& s) {
        auto all=getAll(); std::vector<DvelopEntry> r;
        for(auto& e:all) if(!e.deleted && e.status==s) r.push_back(e);
        return r;
    }
    
    std::vector<DvelopEntry> getRecent(int limit=50) {
        auto all=getAll();
...
class DvelopService {
public:
    static DvelopService& instance() { static DvelopService svc; return svc; }
    
    void init() { initialized_=true; }
    bool isInitialized() const { return initialized_; }
    
    bool processEntry(const DvelopEntry& entry) {
        spdlog::info("dvelop: Processing entry: {}", entry.title);
        return true;
    }
    
    std::string getStatus() const {
        return "{ \"domain\": \"" + std::string("dvelop") + "\", \"status\": \"OK\" }";
    }
