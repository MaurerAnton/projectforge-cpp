...
struct PrivacyprotectionConfig {
    bool enabled = true;
    std::string name;
...
public:
    ...
    std::vector<PrivacyprotectionEntry> getByStatus(const std::string& s) {
        auto all=getAll(); std::vector<PrivacyprotectionEntry> r;
        for(auto& e:all) if(!e.deleted && e.status==s) r.push_back(e);
        return r;
    }
    
    std::vector<PrivacyprotectionEntry> getRecent(int limit=50) {
        auto all=getAll();
...
public:
    static PrivacyprotectionService& instance() { static PrivacyprotectionService svc; return svc; }
    
    void init() { initialized_=true; }
    bool isInitialized() const { return initialized_; }
    
    bool processEntry(const PrivacyprotectionEntry& entry) {
        spdlog::info("privacyprotection: Processing entry: {}", entry.title);
        return true;
    }
    
    std::string getStatus() const {
        return "{ \"domain\": \"" + std::string("privacyprotection") + "\", \"status\": \"OK\" }";
    }
...
private:
    ...
    PrivacyprotectionService()=default;
};
