...
public:
    ...
    std::vector<SystemEntry> getByStatus(const std::string& s) {
        auto all=getAll(); std::vector<SystemEntry> r;
        for(auto& e:all) if(!e.deleted && e.status==s) r.push_back(e);
        return r;
    }
    
    std::vector<SystemEntry> getRecent(int limit=50) {
        auto all=getAll();
...
class SystemService {
public:
    static SystemService& instance() { static SystemService svc; return svc; }
    
    void init() { initialized_=true; }
    bool isInitialized() const { return initialized_; }
    
    bool processEntry(const SystemEntry& entry) {
        spdlog::info("system: Processing entry: {}", entry.title);
        return true;
    }
    
    std::string getStatus() const {
        return "{ \"domain\": \"" + std::string("system") + "\", \"status\": \"OK\" }";
    }
