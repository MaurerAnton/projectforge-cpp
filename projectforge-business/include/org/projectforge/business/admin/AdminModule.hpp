...
public:
    ...
    std::vector<AdminEntry> getByStatus(const std::string& s) {
        auto all=getAll(); std::vector<AdminEntry> r;
        for(auto& e:all) if(!e.deleted && e.status==s) r.push_back(e);
        return r;
    }
    
    std::vector<AdminEntry> getRecent(int limit=50) {
        auto all=getAll();
...
class AdminService {
public:
    static AdminService& instance() { static AdminService svc; return svc; }
    
    void init() { initialized_=true; }
    bool isInitialized() const { return initialized_; }
    
    bool processEntry(const AdminEntry& entry) {
        spdlog::info("admin: Processing entry: {}", entry.title);
        return true;
    }
    
    std::string getStatus() const {
        return "{ \"domain\": \"" + std::string("admin") + "\", \"status\": \"OK\" }";
    }
