...
public:
    ...
    std::vector<LoginEntry> getByStatus(const std::string& s) {
        auto all=getAll(); std::vector<LoginEntry> r;
        for(auto& e:all) if(!e.deleted && e.status==s) r.push_back(e);
        return r;
    }
    
    std::vector<LoginEntry> getRecent(int limit=50) {
        auto all=getAll();
...
class LoginService {
public:
    static LoginService& instance() { static LoginService svc; return svc; }
    
    void init() { initialized_=true; }
    bool isInitialized() const { return initialized_; }
    
    bool processEntry(const LoginEntry& entry) {
        spdlog::info("login: Processing entry: {}", entry.title);
        return true;
    }
    
    std::string getStatus() const {
        return "{ \"domain\": \"" + std::string("login") + "\", \"status\": \"OK\" }";
    }
