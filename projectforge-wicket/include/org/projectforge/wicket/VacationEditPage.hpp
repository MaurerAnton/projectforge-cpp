...
class VacationEditPage : public BaseEditPage {
public:
    VacationEditPage(int64_t id = -1) : BaseEditPage("ProjectForge - Edit Vacation", "Edit Vacation"), entityId_(id) {
        buildForm();
    }
    ...
    static std::vector<std::string> getFieldNames() {
        return { "user", "startDate", "endDate", "type", "comment" };
    }
