...
class AddressEditPage : public BaseEditPage {
public:
    AddressEditPage(int64_t id = -1) : BaseEditPage("ProjectForge - Edit Address", "Edit Address"), entityId_(id) {
        buildForm();
    }
    ...
    static std::vector<std::string> getFieldNames() {
        return { "name", "firstName", "lastName", "organization", "email", "phone", "street", "city" };
    }
