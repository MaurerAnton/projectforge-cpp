...
class FibuRechnungEditPage : public BaseEditPage {
public:
    FibuRechnungEditPage(int64_t id = -1) : BaseEditPage("ProjectForge - Edit Invoice", "Edit Invoice"), entityId_(id) {
        buildForm();
    }
    ...
    static std::vector<std::string> getFieldNames() {
        return { "subject", "customer", "date", "paymentType", "positions", "comment" };
    }
