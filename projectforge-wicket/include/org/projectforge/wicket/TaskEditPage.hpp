...
class TaskEditPage : public BaseEditPage {
public:
    TaskEditPage(int64_t id = -1) : BaseEditPage("ProjectForge - Edit Task", "Edit Task"), entityId_(id) {
        buildForm();
    }
    ...
    static std::vector<std::string> getFieldNames() {
        return { "title", "description", "status", "priority", "responsible", "deadline" };
    }
