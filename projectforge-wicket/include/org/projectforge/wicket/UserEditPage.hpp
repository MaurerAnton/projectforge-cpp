...
class UserEditPage : public BaseEditPage {
public:
    UserEditPage(int64_t id = -1) : BaseEditPage("ProjectForge - Edit User", "Edit User"), entityId_(id) {
        buildForm();
    }
    ...
    static std::vector<std::string> getFieldNames() {
        return { "username", "firstName", "lastName", "email", "locale", "deactivated" };
    }
