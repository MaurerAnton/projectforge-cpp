...
class BookEditPage : public BaseEditPage {
public:
    BookEditPage(int64_t id = -1) : BaseEditPage("ProjectForge - Edit Book", "Edit Book"), entityId_(id) {
        buildForm();
    }
    ...
    static std::vector<std::string> getFieldNames() {
        return { "title", "author", "isbn", "year", "publisher", "available" };
    }
