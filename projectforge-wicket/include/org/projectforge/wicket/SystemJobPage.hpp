...
class GroupEditPage : public BaseEditPage {
public:
    GroupEditPage(int64_t id = -1) : BaseEditPage("ProjectForge - Edit Group", "Edit Group"), entityId_(id) {
        buildForm();
    }
    ...
    static std::vector<std::string> getFieldNames() {
        return { "name", "description", "localGroup" };
    }
