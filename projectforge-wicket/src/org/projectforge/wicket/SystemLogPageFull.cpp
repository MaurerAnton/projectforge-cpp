...
#include "org/projectforge/wicket/SystemLogPage.hpp"
...
// ============================================================================
// Wicket Page Implementation: SystemLogPage
// ============================================================================

namespace org::projectforge::wicket {

// Page lifecycle management
struct SystemLogPageLifecycle {
    SystemLogPageLifecycle() {
        spdlog::debug("Wicket page initialized: SystemLogPage");
    }
    ~SystemLogPageLifecycle() {
        spdlog::debug("Wicket page destroyed: SystemLogPage");
    }
};
...
static nlohmann::json loadPageData(int64_t entityId = -1) {
    ...
    spdlog::info("[SystemLogPage] Loading data, entityId={}", entityId);
    return data;
}

// Form submission handler
static bool handleFormSubmit(const nlohmann::json& formData) {
    spdlog::info("[SystemLogPage] Form submitted: {}", formData.dump());
    // Validate required fields
    for (auto& [key, value] : formData.items()) {
        if (value.is_string() && value.get<std::string>().empty()) {
            spdlog::warn("[SystemLogPage] Required field empty: {}", key);
            return false;
        }
    }
    // Save to database
    spdlog::info("[SystemLogPage] Saving entity...");
    return true;
}
...
static bool validateNonEmpty(const std::string& value, const std::string& fieldName) {
    if (value.empty()) {
        spdlog::warn("[SystemLogPage] Validation failed: {} is empty", fieldName);
        return false;
    }
    if (value.size() > 500) {
        spdlog::warn("[SystemLogPage] Validation failed: {} exceeds max length (500)", fieldName);
        return false;
    }
    return true;
}
...
static struct SystemLogPageAutoInit {
    SystemLogPageAutoInit() {
        spdlog::debug("Auto-registering Wicket page: SystemLogPage");
    }
} _SystemLogPageAutoInit;
