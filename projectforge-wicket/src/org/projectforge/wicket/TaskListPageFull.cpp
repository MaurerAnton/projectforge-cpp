...
#include "org/projectforge/wicket/TaskListPage.hpp"
...
// ============================================================================
// Wicket Page Implementation: TaskListPage
// ============================================================================

namespace org::projectforge::wicket {

// Page lifecycle management
struct TaskListPageLifecycle {
    TaskListPageLifecycle() {
        spdlog::debug("Wicket page initialized: TaskListPage");
    }
    ~TaskListPageLifecycle() {
        spdlog::debug("Wicket page destroyed: TaskListPage");
    }
};
...
static nlohmann::json loadPageData(int64_t entityId = -1) {
    ...
    spdlog::info("[TaskListPage] Loading data, entityId={}", entityId);
    return data;
}

// Form submission handler
static bool handleFormSubmit(const nlohmann::json& formData) {
    spdlog::info("[TaskListPage] Form submitted: {}", formData.dump());
    // Validate required fields
    for (auto& [key, value] : formData.items()) {
        if (value.is_string() && value.get<std::string>().empty()) {
            spdlog::warn("[TaskListPage] Required field empty: {}", key);
            return false;
        }
    }
    // Save to database
    spdlog::info("[TaskListPage] Saving entity...");
    return true;
}
...
static bool validateNonEmpty(const std::string& value, const std::string& fieldName) {
    if (value.empty()) {
        spdlog::warn("[TaskListPage] Validation failed: {} is empty", fieldName);
        return false;
    }
    if (value.size() > 500) {
        spdlog::warn("[TaskListPage] Validation failed: {} exceeds max length (500)", fieldName);
        return false;
    }
    return true;
}
...
static struct TaskListPageAutoInit {
    TaskListPageAutoInit() {
        spdlog::debug("Auto-registering Wicket page: TaskListPage");
    }
} _TaskListPageAutoInit;
