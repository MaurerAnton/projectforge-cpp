...
#include "org/projectforge/wicket/BookListPage.hpp"
...
// ============================================================================
// Wicket Page Implementation: BookListPage
// ============================================================================

namespace org::projectforge::wicket {

// Page lifecycle management
struct BookListPageLifecycle {
    BookListPageLifecycle() {
        spdlog::debug("Wicket page initialized: BookListPage");
    }
    ~BookListPageLifecycle() {
        spdlog::debug("Wicket page destroyed: BookListPage");
    }
};
...
static nlohmann::json loadPageData(int64_t entityId = -1) {
    ...
    spdlog::info("[BookListPage] Loading data, entityId={}", entityId);
    return data;
}

// Form submission handler
static bool handleFormSubmit(const nlohmann::json& formData) {
    spdlog::info("[BookListPage] Form submitted: {}", formData.dump());
    // Validate required fields
    for (auto& [key, value] : formData.items()) {
        if (value.is_string() && value.get<std::string>().empty()) {
            spdlog::warn("[BookListPage] Required field empty: {}", key);
            return false;
        }
    }
    // Save to database
    spdlog::info("[BookListPage] Saving entity...");
    return true;
}
...
static bool validateNonEmpty(const std::string& value, const std::string& fieldName) {
    if (value.empty()) {
        spdlog::warn("[BookListPage] Validation failed: {} is empty", fieldName);
        return false;
    }
    if (value.size() > 500) {
        spdlog::warn("[BookListPage] Validation failed: {} exceeds max length (500)", fieldName);
        return false;
    }
    return true;
}
...
static struct BookListPageAutoInit {
    BookListPageAutoInit() {
        spdlog::debug("Auto-registering Wicket page: BookListPage");
    }
} _BookListPageAutoInit;
