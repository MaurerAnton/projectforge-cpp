...
#include "org/projectforge/wicket/SystemBackupPage.hpp"
...
// ============================================================================
// Wicket Page Implementation: SystemBackupPage
// ============================================================================

namespace org::projectforge::wicket {

// Page lifecycle management
struct SystemBackupPageLifecycle {
    SystemBackupPageLifecycle() {
        spdlog::debug("Wicket page initialized: SystemBackupPage");
    }
    ~SystemBackupPageLifecycle() {
        spdlog::debug("Wicket page destroyed: SystemBackupPage");
    }
};
...
static nlohmann::json loadPageData(int64_t entityId = -1) {
    ...
    spdlog::info("[SystemBackupPage] Loading data, entityId={}", entityId);
    return data;
}

// Form submission handler
static bool handleFormSubmit(const nlohmann::json& formData) {
    spdlog::info("[SystemBackupPage] Form submitted: {}", formData.dump());
    // Validate required fields
    for (auto& [key, value] : formData.items()) {
        if (value.is_string() && value.get<std::string>().empty()) {
            spdlog::warn("[SystemBackupPage] Required field empty: {}", key);
            return false;
        }
    }
    // Save to database
    spdlog::info("[SystemBackupPage] Saving entity...");
    return true;
}
...
static bool validateNonEmpty(const std::string& value, const std::string& fieldName) {
    if (value.empty()) {
        spdlog::warn("[SystemBackupPage] Validation failed: {} is empty", fieldName);
        return false;
    }
    if (value.size() > 500) {
        spdlog::warn("[SystemBackupPage] Validation failed: {} exceeds max length (500)", fieldName);
        return false;
    }
    return true;
}
...
static struct SystemBackupPageAutoInit {
    SystemBackupPageAutoInit() {
        spdlog::debug("Auto-registering Wicket page: SystemBackupPage");
    }
} _SystemBackupPageAutoInit;
