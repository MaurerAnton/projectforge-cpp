...
#include "org/projectforge/wicket/CalendarPage.hpp"
...
// ============================================================================
// Wicket Page Implementation: CalendarPage
// ============================================================================

namespace org::projectforge::wicket {

// Page lifecycle management
struct CalendarPageLifecycle {
    CalendarPageLifecycle() {
        spdlog::debug("Wicket page initialized: CalendarPage");
    }
    ~CalendarPageLifecycle() {
        spdlog::debug("Wicket page destroyed: CalendarPage");
    }
};
...
static nlohmann::json loadPageData(int64_t entityId = -1) {
    ...
    spdlog::info("[CalendarPage] Loading data, entityId={}", entityId);
    return data;
}

// Form submission handler
static bool handleFormSubmit(const nlohmann::json& formData) {
    spdlog::info("[CalendarPage] Form submitted: {}", formData.dump());
    // Validate required fields
    for (auto& [key, value] : formData.items()) {
        if (value.is_string() && value.get<std::string>().empty()) {
            spdlog::warn("[CalendarPage] Required field empty: {}", key);
            return false;
        }
    }
    // Save to database
    spdlog::info("[CalendarPage] Saving entity...");
    return true;
}
...
static bool validateNonEmpty(const std::string& value, const std::string& fieldName) {
    if (value.empty()) {
        spdlog::warn("[CalendarPage] Validation failed: {} is empty", fieldName);
        return false;
    }
    if (value.size() > 500) {
        spdlog::warn("[CalendarPage] Validation failed: {} exceeds max length (500)", fieldName);
        return false;
    }
    return true;
}
...
static struct CalendarPageAutoInit {
    CalendarPageAutoInit() {
        spdlog::debug("Auto-registering Wicket page: CalendarPage");
    }
} _CalendarPageAutoInit;
