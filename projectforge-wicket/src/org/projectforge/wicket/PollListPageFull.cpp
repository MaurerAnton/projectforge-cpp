...
#include "org/projectforge/wicket/PollListPage.hpp"
...
// ============================================================================
// Wicket Page Implementation: PollListPage
// ============================================================================

namespace org::projectforge::wicket {

// Page lifecycle management
struct PollListPageLifecycle {
    PollListPageLifecycle() {
        spdlog::debug("Wicket page initialized: PollListPage");
    }
    ~PollListPageLifecycle() {
        spdlog::debug("Wicket page destroyed: PollListPage");
    }
};
...
static nlohmann::json loadPageData(int64_t entityId = -1) {
    ...
    spdlog::info("[PollListPage] Loading data, entityId={}", entityId);
    return data;
}

// Form submission handler
static bool handleFormSubmit(const nlohmann::json& formData) {
    spdlog::info("[PollListPage] Form submitted: {}", formData.dump());
    // Validate required fields
    for (auto& [key, value] : formData.items()) {
        if (value.is_string() && value.get<std::string>().empty()) {
            spdlog::warn("[PollListPage] Required field empty: {}", key);
            return false;
        }
    }
    // Save to database
    spdlog::info("[PollListPage] Saving entity...");
    return true;
}
...
static bool validateNonEmpty(const std::string& value, const std::string& fieldName) {
    if (value.empty()) {
        spdlog::warn("[PollListPage] Validation failed: {} is empty", fieldName);
        return false;
    }
    if (value.size() > 500) {
        spdlog::warn("[PollListPage] Validation failed: {} exceeds max length (500)", fieldName);
        return false;
    }
    return true;
}
...
static struct PollListPageAutoInit {
    PollListPageAutoInit() {
        spdlog::debug("Auto-registering Wicket page: PollListPage");
    }
} _PollListPageAutoInit;
