...
#include "org/projectforge/wicket/TaskEditPage.hpp"
...
// ============================================================================
// Wicket Page Implementation: TaskEditPage
// ============================================================================

namespace org::projectforge::wicket {

// Page lifecycle management
struct TaskEditPageLifecycle {
    TaskEditPageLifecycle() {
        spdlog::debug("Wicket page initialized: TaskEditPage");
    }
    ~TaskEditPageLifecycle() {
        spdlog::debug("Wicket page destroyed: TaskEditPage");
    }
};
...
static nlohmann::json loadPageData(int64_t entityId = -1) {
    ...
    spdlog::info("[TaskEditPage] Loading data, entityId={}", entityId);
    return data;
}

// Form submission handler
static bool handleFormSubmit(const nlohmann::json& formData) {
    spdlog::info("[TaskEditPage] Form submitted: {}", formData.dump());
    // Validate required fields
    for (auto& [key, value] : formData.items()) {
        if (value.is_string() && value.get<std::string>().empty()) {
            spdlog::warn("[TaskEditPage] Required field empty: {}", key);
            return false;
        }
    }
    // Save to database
    spdlog::info("[TaskEditPage] Saving entity...");
    return true;
}
...
static bool validateNonEmpty(const std::string& value, const std::string& fieldName) {
    if (value.empty()) {
        spdlog::warn("[TaskEditPage] Validation failed: {} is empty", fieldName);
        return false;
    }
    if (value.size() > 500) {
        spdlog::warn("[TaskEditPage] Validation failed: {} exceeds max length (500)", fieldName);
        return false;
    }
    return true;
}
...
static struct TaskEditPageAutoInit {
    TaskEditPageAutoInit() {
        spdlog::debug("Auto-registering Wicket page: TaskEditPage");
    }
} _TaskEditPageAutoInit;
