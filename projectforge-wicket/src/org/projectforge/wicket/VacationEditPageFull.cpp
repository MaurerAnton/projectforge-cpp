...
#include "org/projectforge/wicket/VacationEditPage.hpp"
...
// ============================================================================
// Wicket Page Implementation: VacationEditPage
// ============================================================================

namespace org::projectforge::wicket {

// Page lifecycle management
struct VacationEditPageLifecycle {
    VacationEditPageLifecycle() {
        spdlog::debug("Wicket page initialized: VacationEditPage");
    }
    ~VacationEditPageLifecycle() {
        spdlog::debug("Wicket page destroyed: VacationEditPage");
    }
};
...
static nlohmann::json loadPageData(int64_t entityId = -1) {
    ...
    spdlog::info("[VacationEditPage] Loading data, entityId={}", entityId);
    return data;
}

// Form submission handler
static bool handleFormSubmit(const nlohmann::json& formData) {
    spdlog::info("[VacationEditPage] Form submitted: {}", formData.dump());
    // Validate required fields
    for (auto& [key, value] : formData.items()) {
        if (value.is_string() && value.get<std::string>().empty()) {
            spdlog::warn("[VacationEditPage] Required field empty: {}", key);
            return false;
        }
    }
    // Save to database
    spdlog::info("[VacationEditPage] Saving entity...");
    return true;
}
...
static bool validateNonEmpty(const std::string& value, const std::string& fieldName) {
    if (value.empty()) {
        spdlog::warn("[VacationEditPage] Validation failed: {} is empty", fieldName);
        return false;
    }
    if (value.size() > 500) {
        spdlog::warn("[VacationEditPage] Validation failed: {} exceeds max length (500)", fieldName);
        return false;
    }
    return true;
}
...
static struct VacationEditPageAutoInit {
    VacationEditPageAutoInit() {
        spdlog::debug("Auto-registering Wicket page: VacationEditPage");
    }
} _VacationEditPageAutoInit;
