...
#include "org/projectforge/wicket/GroupEditPage.hpp"
...
// ============================================================================
// Wicket Page Implementation: GroupEditPage
// ============================================================================

namespace org::projectforge::wicket {

// Page lifecycle management
struct GroupEditPageLifecycle {
    GroupEditPageLifecycle() {
        spdlog::debug("Wicket page initialized: GroupEditPage");
    }
    ~GroupEditPageLifecycle() {
        spdlog::debug("Wicket page destroyed: GroupEditPage");
    }
};
...
static nlohmann::json loadPageData(int64_t entityId = -1) {
    ...
    spdlog::info("[GroupEditPage] Loading data, entityId={}", entityId);
    return data;
}

// Form submission handler
static bool handleFormSubmit(const nlohmann::json& formData) {
    spdlog::info("[GroupEditPage] Form submitted: {}", formData.dump());
    // Validate required fields
    for (auto& [key, value] : formData.items()) {
        if (value.is_string() && value.get<std::string>().empty()) {
            spdlog::warn("[GroupEditPage] Required field empty: {}", key);
            return false;
        }
    }
    // Save to database
    spdlog::info("[GroupEditPage] Saving entity...");
    return true;
}
...
static bool validateNonEmpty(const std::string& value, const std::string& fieldName) {
    if (value.empty()) {
        spdlog::warn("[GroupEditPage] Validation failed: {} is empty", fieldName);
        return false;
    }
    if (value.size() > 500) {
        spdlog::warn("[GroupEditPage] Validation failed: {} exceeds max length (500)", fieldName);
        return false;
    }
    return true;
}
...
static struct GroupEditPageAutoInit {
    GroupEditPageAutoInit() {
        spdlog::debug("Auto-registering Wicket page: GroupEditPage");
    }
} _GroupEditPageAutoInit;
