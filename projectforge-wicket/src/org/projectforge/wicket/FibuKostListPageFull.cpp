...
#include "org/projectforge/wicket/FibuKostListPage.hpp"
...
// ============================================================================
// Wicket Page Implementation: FibuKostListPage
// ============================================================================

namespace org::projectforge::wicket {

// Page lifecycle management
struct FibuKostListPageLifecycle {
    FibuKostListPageLifecycle() {
        spdlog::debug("Wicket page initialized: FibuKostListPage");
    }
    ~FibuKostListPageLifecycle() {
        spdlog::debug("Wicket page destroyed: FibuKostListPage");
    }
};
...
static nlohmann::json loadPageData(int64_t entityId = -1) {
    ...
    spdlog::info("[FibuKostListPage] Loading data, entityId={}", entityId);
    return data;
}

// Form submission handler
static bool handleFormSubmit(const nlohmann::json& formData) {
    spdlog::info("[FibuKostListPage] Form submitted: {}", formData.dump());
    // Validate required fields
    for (auto& [key, value] : formData.items()) {
        if (value.is_string() && value.get<std::string>().empty()) {
            spdlog::warn("[FibuKostListPage] Required field empty: {}", key);
            return false;
        }
    }
    // Save to database
    spdlog::info("[FibuKostListPage] Saving entity...");
    return true;
}
...
static bool validateNonEmpty(const std::string& value, const std::string& fieldName) {
    if (value.empty()) {
        spdlog::warn("[FibuKostListPage] Validation failed: {} is empty", fieldName);
        return false;
    }
    if (value.size() > 500) {
        spdlog::warn("[FibuKostListPage] Validation failed: {} exceeds max length (500)", fieldName);
        return false;
    }
    return true;
}
...
static struct FibuKostListPageAutoInit {
    FibuKostListPageAutoInit() {
        spdlog::debug("Auto-registering Wicket page: FibuKostListPage");
    }
} _FibuKostListPageAutoInit;
