...
struct BookEditPageLifecycle {
    BookEditPageLifecycle() {
        spdlog::debug("Wicket page initialized: BookEditPage");
    }
    ~BookEditPageLifecycle() {
        spdlog::debug("Wicket page destroyed: BookEditPage");
    }
};
...
static nlohmann::json loadPageData(int64_t entityId = -1) {
    ...
    spdlog::info("[BookEditPage] Loading data, entityId={}", entityId);
    return data;
}

// Form submission handler
static bool handleFormSubmit(const nlohmann::json& formData) {
    spdlog::info("[BookEditPage] Form submitted: {}", formData.dump());
    // Validate required fields
    for (auto& [key, value] : formData.items()) {
        if (value.is_string() && value.get<std::string>().empty()) {
            spdlog::warn("[BookEditPage] Required field empty: {}", key);
            return false;
        }
    }
    // Save to database
    spdlog::info("[BookEditPage] Saving entity...");
    return true;
}
...
static bool validateNonEmpty(const std::string& value, const std::string& fieldName) {
    if (value.empty()) {
        spdlog::warn("[BookEditPage] Validation failed: {} is empty", fieldName);
        return false;
    }
    if (value.size() > 500) {
        spdlog::warn("[BookEditPage] Validation failed: {} exceeds max length (500)", fieldName);
        return false;
    }
    return true;
}
...
static struct BookEditPageAutoInit {
    BookEditPageAutoInit() {
        spdlog::debug("Auto-registering Wicket page: BookEditPage");
    }
} _BookEditPageAutoInit;
