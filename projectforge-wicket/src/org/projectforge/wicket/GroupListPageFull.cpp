...
#include "org/projectforge/wicket/GroupListPage.hpp"
...
// ============================================================================
// Wicket Page Implementation: GroupListPage
// ============================================================================

namespace org::projectforge::wicket {

// Page lifecycle management
struct GroupListPageLifecycle {
    GroupListPageLifecycle() {
        spdlog::debug("Wicket page initialized: GroupListPage");
    }
    ~GroupListPageLifecycle() {
        spdlog::debug("Wicket page destroyed: GroupListPage");
    }
};
...
static nlohmann::json loadPageData(int64_t entityId = -1) {
    ...
    spdlog::info("[GroupListPage] Loading data, entityId={}", entityId);
    return data;
}

// Form submission handler
static bool handleFormSubmit(const nlohmann::json& formData) {
    spdlog::info("[GroupListPage] Form submitted: {}", formData.dump());
    // Validate required fields
    for (auto& [key, value] : formData.items()) {
        if (value.is_string() && value.get<std::string>().empty()) {
            spdlog::warn("[GroupListPage] Required field empty: {}", key);
            return false;
        }
    }
    // Save to database
    spdlog::info("[GroupListPage] Saving entity...");
    return true;
}
...
static bool validateNonEmpty(const std::string& value, const std::string& fieldName) {
    if (value.empty()) {
        spdlog::warn("[GroupListPage] Validation failed: {} is empty", fieldName);
        return false;
    }
    if (value.size() > 500) {
        spdlog::warn("[GroupListPage] Validation failed: {} exceeds max length (500)", fieldName);
        return false;
    }
    return true;
}
...
static struct GroupListPageAutoInit {
    GroupListPageAutoInit() {
        spdlog::debug("Auto-registering Wicket page: GroupListPage");
    }
} _GroupListPageAutoInit;
