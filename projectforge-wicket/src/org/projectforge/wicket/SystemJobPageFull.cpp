...
#include "org/projectforge/wicket/SystemJobPage.hpp"
...
// ============================================================================
// Wicket Page Implementation: SystemJobPage
// ============================================================================

namespace org::projectforge::wicket {

// Page lifecycle management
struct SystemJobPageLifecycle {
    SystemJobPageLifecycle() {
        spdlog::debug("Wicket page initialized: SystemJobPage");
    }
    ~SystemJobPageLifecycle() {
        spdlog::debug("Wicket page destroyed: SystemJobPage");
    }
};
...
static nlohmann::json loadPageData(int64_t entityId = -1) {
    ...
    spdlog::info("[SystemJobPage] Loading data, entityId={}", entityId);
    return data;
}

// Form submission handler
static bool handleFormSubmit(const nlohmann::json& formData) {
    spdlog::info("[SystemJobPage] Form submitted: {}", formData.dump());
    // Validate required fields
    for (auto& [key, value] : formData.items()) {
        if (value.is_string() && value.get<std::string>().empty()) {
            spdlog::warn("[SystemJobPage] Required field empty: {}", key);
            return false;
        }
    }
    // Save to database
    spdlog::info("[SystemJobPage] Saving entity...");
    return true;
}
...
static bool validateNonEmpty(const std::string& value, const std::string& fieldName) {
    if (value.empty()) {
        spdlog::warn("[SystemJobPage] Validation failed: {} is empty", fieldName);
        return false;
    }
    if (value.size() > 500) {
        spdlog::warn("[SystemJobPage] Validation failed: {} exceeds max length (500)", fieldName);
        return false;
    }
    return true;
}
...
static struct SystemJobPageAutoInit {
    SystemJobPageAutoInit() {
        spdlog::debug("Auto-registering Wicket page: SystemJobPage");
    }
} _SystemJobPageAutoInit;
