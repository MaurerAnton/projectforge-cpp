...
#include "org/projectforge/wicket/TaskTreePage.hpp"
...
// ============================================================================
// Wicket Page Implementation: TaskTreePage
// ============================================================================

namespace org::projectforge::wicket {

// Page lifecycle management
struct TaskTreePageLifecycle {
    TaskTreePageLifecycle() {
        spdlog::debug("Wicket page initialized: TaskTreePage");
    }
    ~TaskTreePageLifecycle() {
        spdlog::debug("Wicket page destroyed: TaskTreePage");
    }
};
...
static nlohmann::json loadPageData(int64_t entityId = -1) {
    ...
    spdlog::info("[TaskTreePage] Loading data, entityId={}", entityId);
    return data;
}

// Form submission handler
static bool handleFormSubmit(const nlohmann::json& formData) {
    spdlog::info("[TaskTreePage] Form submitted: {}", formData.dump());
    // Validate required fields
    for (auto& [key, value] : formData.items()) {
        if (value.is_string() && value.get<std::string>().empty()) {
            spdlog::warn("[TaskTreePage] Required field empty: {}", key);
            return false;
        }
    }
    // Save to database
    spdlog::info("[TaskTreePage] Saving entity...");
    return true;
}
...
static bool validateNonEmpty(const std::string& value, const std::string& fieldName) {
    if (value.empty()) {
        spdlog::warn("[TaskTreePage] Validation failed: {} is empty", fieldName);
        return false;
    }
    if (value.size() > 500) {
        spdlog::warn("[TaskTreePage] Validation failed: {} exceeds max length (500)", fieldName);
        return false;
    }
    return true;
}
...
static struct TaskTreePageAutoInit {
    TaskTreePageAutoInit() {
        spdlog::debug("Auto-registering Wicket page: TaskTreePage");
    }
} _TaskTreePageAutoInit;
