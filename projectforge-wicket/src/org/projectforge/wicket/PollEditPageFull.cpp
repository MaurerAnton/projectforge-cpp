...
#include "org/projectforge/wicket/PollEditPage.hpp"
...
// ============================================================================
// Wicket Page Implementation: PollEditPage
// ============================================================================

namespace org::projectforge::wicket {

// Page lifecycle management
struct PollEditPageLifecycle {
    PollEditPageLifecycle() {
        spdlog::debug("Wicket page initialized: PollEditPage");
    }
    ~PollEditPageLifecycle() {
        spdlog::debug("Wicket page destroyed: PollEditPage");
    }
};
...
static nlohmann::json loadPageData(int64_t entityId = -1) {
    ...
    spdlog::info("[PollEditPage] Loading data, entityId={}", entityId);
    return data;
}

// Form submission handler
static bool handleFormSubmit(const nlohmann::json& formData) {
    spdlog::info("[PollEditPage] Form submitted: {}", formData.dump());
    // Validate required fields
    for (auto& [key, value] : formData.items()) {
        if (value.is_string() && value.get<std::string>().empty()) {
            spdlog::warn("[PollEditPage] Required field empty: {}", key);
            return false;
        }
    }
    // Save to database
    spdlog::info("[PollEditPage] Saving entity...");
    return true;
}
...
static bool validateNonEmpty(const std::string& value, const std::string& fieldName) {
    if (value.empty()) {
        spdlog::warn("[PollEditPage] Validation failed: {} is empty", fieldName);
        return false;
    }
    if (value.size() > 500) {
        spdlog::warn("[PollEditPage] Validation failed: {} exceeds max length (500)", fieldName);
        return false;
    }
    return true;
}
...
static struct PollEditPageAutoInit {
    PollEditPageAutoInit() {
        spdlog::debug("Auto-registering Wicket page: PollEditPage");
    }
} _PollEditPageAutoInit;
